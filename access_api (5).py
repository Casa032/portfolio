...
# --- Réglage du débit vers lex-api.com (évite les HTTP 429) ---
DELAI_CELEX = 1.0      # secondes d'attente entre deux requêtes détail
MAX_RETRIES_CELEX = 3  # tentatives en cas de 429 avant d'abandonner
...
def recup_contenu_celex(celex: str, headers: dict = None) -> str | None:
    ...
    Throttling : on attend DELAI_CELEX avant chaque appel, et on réessaie
    en cas de 429 (Too Many Requests) en respectant l'en-tête Retry-After.
    """
    if not celex:
        return None
    ...
    for tentative in range(1, MAX_RETRIES_CELEX + 1):
        time.sleep(DELAI_CELEX)
        try:
            r = requests.post(URL_DETAIL, json=payload, headers=headers or {}, timeout=30)

            if r.status_code == 429:
                # l'API demande de ralentir : on respecte Retry-After si fourni
                attente = float(r.headers.get("Retry-After", DELAI_CELEX * tentative * 2))
                print(f"  · 429 sur {celex}, nouvelle tentative dans {attente:.0f}s "
                      f"({tentative}/{MAX_RETRIES_CELEX})")
                time.sleep(attente)
                continue

            r.raise_for_status()
            return r.json().get("content", {}).get("fullText")

        except Exception as e:
            print(f"  ! contenu CELEX {celex} indisponible : {e}")
            return None

    print(f"  ! contenu CELEX {celex} abandonné après {MAX_RETRIES_CELEX} tentatives (429)")
    return None
...
def recup_contenu_jorf(cid: str, token: str) -> str | None:
    ...
    try:
        ...
        # La réponse JORF expose le texte dans une LISTE "articles",
        # chaque élément ayant un champ "content". Un texte peut être
        # découpé en plusieurs articles : on les concatène tous.
        articles = data.get("articles") or []
        morceaux = [a.get("content") for a in articles if a.get("content")]
        if morceaux:
            return "\n\n".join(morceaux)

        # Repli : certains textes exposent le contenu ailleurs
        return data.get("texte") or data.get("contenu") or None
...
def recup_jo(source: dict, client_id: str, client_secret: str, mots_cles: list, nb: int = 300,
             date_min: str = "2026-06-01") -> list[dict]:
    """
    Récupère les derniers JO, filtre par mots-clés et ne garde que les JO
    publiés à partir de date_min (format AAAA-MM-JJ, borne incluse).
    datePubli est un timestamp en millisecondes dans la réponse PISTE.
    """
    ...
    # Seuil en millisecondes (minuit UTC du jour date_min)
    seuil_ms = None
    if date_min:
        seuil_ms = int(
    ...
    for container in result.get("containers", []):
        ...
        # Filtre par date : on saute les JO antérieurs à date_min.
        # (le saut évite aussi les appels recup_contenu_jorf inutiles)
        if seuil_ms is not None and isinstance(date_jo, (int, float)) and date_jo < seuil_ms:
            ignores += 1
            continue
    ...
    print(f"API-{nom} -> {len(articles)} articles récupérés "
          f"({ignores} JO ignorés avant {date_min})")
    return articles
