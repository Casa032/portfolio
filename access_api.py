...
def recup_contenu_celex(celex: str, headers: dict = None) -> str | None:
    ...
    try:
        ...
        return data.get("content") or data.get("text") or data.get("html")
...
def recup_api(source: dict, mapping: dict, params: dict = None, headers: dict = None) -> list[dict]:
    ...
    for item in items:
        ...
        # Seconde requête : contenu intégral via le CELEX extrait de l'URL.
        # cle_dedup renvoie "celex:XXXX" pour les URLs Eur-lex.
        cle = cle_dedup(url or "")
        if cle.startswith("celex:"):
            brut["contenu"] = recup_contenu_celex(cle.split(":", 1)[1], headers=headers)
