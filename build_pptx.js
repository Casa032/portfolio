...
data.parties.forEach((part, pIdx) => {
  ...
  if (part.articles.length === 0) {
    ...
    part.articles.forEach((art, i) => {
      ...
      som.addText([
        ...
        hyperlink: { slide: articleSlides[i], tooltip: "Aller à l'article" },
      });
    });
  }
  ...
  part.articles.forEach((art, i) => {
    ...
    s.addText("↑ Sommaire", {
      ...
      fontFace: "Calibri", margin: 0,
      hyperlink: { slide: plan[pIdx].sommaireSlide, tooltip: "Retour au sommaire" },
    });
    ...
    if (art.url) {
      s.addText("Lire l'article original →", {
        ...
        fontFace: "Calibri", margin: 0,
        hyperlink: { url: art.url, tooltip: art.url },
      });
    }
  });
});

pres.writeFile({ fileName: "rapport_veille.pptx" }).then(() => console.log("pptx écrit"));
