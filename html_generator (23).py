...
Object.values(DATA.historiques||{}).forEach(rows=>{
  ...
  document.getElementById("page-collabs").innerHTML=`
    ...
    <div class="card">
      <div class="card-title">sujets :: ${esc(sel)} (${detail.length})</div>
      <div class="proj-list">
        ${(()=>{
        ...
        const projItemFn=p=>{
          ...
          // Partage prochain point — surbrillance + badge
          if(aPartager){
            const id=p.projet_id||p.ref_sujet;
        ...
        // Tous les sujets à égalité (aucune priorité de type), déjà triés par statut
        return detail.length
          ? detail.map(projItemFn).join("")
          : '<div style="color:var(--text3);font-size:11px;font-family:var(--font-mono);padding:8px">// aucun sujet</div>';
        })()}
      </div>
    </div>`;
}
