...
@keyframes partage-pulse{0%,100%{opacity:1;}50%{opacity:.7;}}.proj-dot{width:6px;height:6px;border-radius:50%;flex-shrink:0;}
...
.modal-text{font-size:12px;color:var(--text2);line-height:1.7;white-space:pre-line;overflow-wrap:anywhere;}
...
const PAGES={overview:"Vue d'ensemble",domaines:"Analyse par domaine",collabs:"Analyse par collaborateur",gantt:"Roadmap",evolutions:"Évolution",calendrier:"Calendrier",stats:"Statistiques"};
let selCollab=null;
let selEntiteGantt="";
...
const NP=(DATA.meta||[]).filter(m=>{
  ...
  // ── Approche "tous types de sujets" ───────────────────────────────
  const norm=s=>String(s||"").toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim();
  const typeLabel=t=>{const u=norm(t);return (u===""||u==="NAN")?"NON DÉFINI":u;};

  // ── STRUCTUREL → depuis le RÉFÉRENTIEL (META), source de vérité ────
  const META=DATA.meta||[];
  // Total de sujets = périmètre du référentiel
  const nbSujetsActifs=META.length;

  // Ventilation par type depuis le référentiel (s'adapte aux types ajoutés à la main)
  const ventil={};
  META.forEach(m=>{const t=typeLabel(m.type);ventil[t]=(ventil[t]||0)+1;});
  const ventilTri=Object.entries(ventil).sort((a,b)=>b[1]-a[1]);

  // ── STATUT → depuis les FICHES de quinzaine (P) ────────────────────
  // (le statut/avancement n'existe que dans les fiches, pas dans le référentiel)
  ...
  // Dates portées par le référentiel ; on les lit sur META
  const now=new Date();
  const h30=new Date(now.getTime()+30*24*3600*1000);
  function dEch(x){return parseDateNP(x.date_prevision||x.date_fin);}
  const echeancesProches=META.filter(m=>{const d=dEch(m);return d&&d>=now&&d<=h30;});
  ...
  document.getElementById("page-overview").innerHTML=`
    <div class="metrics-row" style="grid-template-columns:repeat(6,1fr)">
      <div class="metric-card c-cyan">
        <div class="metric-label">sujets suivis</div>
        <div class="metric-value">${nbSujetsActifs}</div>
        <div class="metric-sub">référentiel · tous types</div>
      </div>
...
function renderCollabs(sel){
  ...
  // ── Détection d'absence fiable : qui a réellement rempli une fiche ──
  ...
  // ── Collaborateurs présents uniquement dans l'historique ──────────
  const respActifs=new Set(DATA.projets.map(p=>p.responsable_principal).filter(Boolean).filter(_aRempli));
...
Object.values(DATA.historiques||{}).forEach(rows=>{
  ...
  // Bloc 1 : sujets rattachés au collaborateur dans la quinzaine active
  ...
  if(isAbsent){
    ...
    if(lastQAbs){
      ...
      detailHist=(DATA.snapshots[lastQAbs]?.projets||[])
        ...
        .filter(p=>!dejaVus.has(p.projet_id||p.ref_sujet))
        .sort(_tri);
    }
  }
    ...
    <div class="card">
      ...
      <div class="proj-list">
        ...
        if(isAbsent&&detail.length){
          // Sous-blocs par contributeur (depuis contributeurs_sujet, produit par le parser)
          const parContrib={};
          const sansContrib=[];
          detail.forEach(p=>{
...
function buildGantt(){
  ...
  let projets = DATA.projets.filter(p=>{
    ...
    return deb&&(fin||prev); // visible si début + (date de fin OU date prévisionnelle)
  });
  ...
  Object.entries(groups).sort().forEach(([grp,items],gi)=>{
    ...
    items.forEach(p=>{
      ...
      const fin = parseDate(p.date_fin||m.date_fin);
      const prev = parseDate(p.date_prevision||m.date_prevision);
      // Fin effective de la barre : date de fin si présente, sinon projection
      const finEff = fin || prev;
      const pv  = p.avancement_pct||0;
      const isOver  = finEff < now && p.statut!=="Terminé";
      const isSoon  = finEff > now && (finEff-now)<30*24*3600*1000;
      const x1  = xPos(deb);
      const x2  = Math.max(xPos(finEff), x1+4);
      const bw  = x2-x1;
      ...
      if(bw>0){
        ...
        // Losange de projection (date_prevision)
        if(prev){
          const xp=xPos(prev);
          // n'afficher que si la projection tombe dans la fenêtre visible
          if(xp>=LBL_W-6 && xp<=svgW+6){
            const cyD=ry+ROW_H/2, r=4.5;
            // anomalie seulement si une date de fin existe ET que la projection la dépasse
            const apresFin = fin && prev>fin;
            const prevStr = esc(String(p.date_prevision||m.date_prevision||""));
            const titre = fin
              ? `projection : ${prevStr}${apresFin?" (après la date de fin !)":""}`
              : `projection : ${prevStr} (pas de date de fin ferme)`;
            svg += `<path d="M ${xp} ${cyD-r} L ${xp+r} ${cyD} L ${xp} ${cyD+r} L ${xp-r} ${cyD} Z"
              fill="${apresFin?"var(--red)":"var(--amber)"}" stroke="var(--bg2)" stroke-width="1.2">
              <title>${titre}</title>
            </path>`;
          }
        }
