...
def cmd_coverage():
    rows = get_coverage()
    print(f"\n{'Source':<22} {'Portée':<12} {'Pays':<5} {'Art.':>5}  "
...
def cmd_rescore():
    articles = get_articles_a_scorer()
    print(f"{len(articles)} articles à scorer")
    for i, article in enumerate(articles, 1):
        article = scorer_article(article)        # lexical (gratuit)
        article = scorer_llm_article(article)    # LLM (1 appel)
        update_scores(article)
        if i % 10 == 0:
            print(f"  ... {i}/{len(articles)}")
    print(f"✓ {len(articles)} articles re-scorés")
