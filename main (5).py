...
def cmd_fetch():
    ...
    for source in SOURCES:
        ...
        for article in articles:
            ...
            article = enrichir_contenu(article)   # scrape/nettoie le contenu
...
def _traiter_articles(articles, libelle):
    ...
    for i, article in enumerate(articles, 1):
        article = enrichir_contenu(article)      # scrape/nettoie le contenu
        update_contenu(article)                  # sauvegarde le contenu enrichi
        article = scorer_article(article)        # lexical (gratuit)
        article = scorer_llm_article(article)    # LLM (score + résumé)
